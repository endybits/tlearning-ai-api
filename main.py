...
def get_corricular_grid(
    type_skills: List[str] = Query(..., description="Describe una o más habilidades"),
    sector_company: str = Query(..., description="Agro | Industria | Servicios"),
):
    skills = {type_skills[0] if len(type_skills) == 1 else " y ".join(type_skills)}

    prompt = f"""Eres un experto en formación corporativa, con más de 30 años de experiencia potenciando las habilidades de {skills} para el capital humano
